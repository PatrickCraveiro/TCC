  ...
  Clinica.findById(req.params.clinicaCNPJ, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message:
            "Error retrieving Clinica with CNPJ " + req.params.clinicaCNPJ,
        });
      }
    } else res.send(data);
  });
    ...
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message:
              "Error updating Clinica with CNPJ " + req.params.clinicaCNPJ,
          });
        }
      } else res.send(data);
    }
  ...
  Clinica.remove(req.params.clinicaCNPJ, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({
          message:
            "Could not delete Clinica with CNPJ " + req.params.clinicaCNPJ,
        });
      }
